...
public class MGLES11Canvas implements MGLCanvas {
    ...
    private final ArrayList<MRawTexture> mTargetStack = new ArrayList<MRawTexture>();
    ...
    public MGLES11Canvas(GL11 gl, boolean init) {
        mGL = gl;
        if (init) {
...
//        Assert.assertTrue(alpha >= 0 && alpha <= 1);
    ...
    @Override
    public void drawMesh(MBasicTexture tex, int x, int y, int xyBuffer,
    ...
    private void drawTexture(MBasicTexture texture,
    ...
    @Override
    public void drawTexture(MBasicTexture texture, RectF source, RectF target) {
    ...
    @Override
    public void drawTexture(MBasicTexture texture, float[] mTextureTransform,
    ...
    @Override
    public void drawMixed(MBasicTexture from, int toColor, float ratio,
    ...
    private void drawMixed(MBasicTexture from, int toColor,
    ...
    @Override
    public boolean unloadTexture(MBasicTexture t) {
    ...
    public void deleteRecycledResources() {
        synchronized (mUnboundTextures) {
            ...
            if (ids.size() > 0) {
                glDeleteTextures(ids.size(), ids.getInternalArray(), 0);
                ids.clear();
            }

            ids = mDeleteBuffers;
            if (ids.size() > 0) {
                glDeleteBuffers(ids.size(), ids.getInternalArray(), 0);
                ids.clear();
            }
        }
    }
    ...
    private static class ConfigState {
        ...
        public void restore(MGLES11Canvas canvas) {
    ...
    public void dumpStatisticsAndClear() {
        ...
        mCountFillRect = 0;
        mCountDrawLine = 0;
        Log.d(TAG, "<dumpStatisticsAndClear> " + line);
    }
    ...
    private void setRenderTarget(MRawTexture texture) {
        GL11ExtensionPack gl11ep = (GL11ExtensionPack) mGL;

        if (mTargetTexture == null && texture != null) {
            // Create frame buffer when it's invalid, after create, it will always be here
            if (mFrameBuffer[0] == INVALID_FRAME_BUFFER) {
                glGenBuffers(1, mFrameBuffer, 0);
            }
        ...
        if (mTargetTexture != null && texture  == null) {
            ...
            // In order to not create frame buffer every time, not delete it
    ...
    public void endRenderTarget() {
        MRawTexture texture = mTargetStack.remove(mTargetStack.size() - 1);
        setRenderTarget(texture);
        restore(); // restore matrix and alpha
    }

    @Override
    public void beginRenderTarget(MRawTexture texture) {
    ...
    @Override
    public void setTextureParameters(MBasicTexture texture) {
        int width = texture.getWidth();
        int height = texture.getHeight();
    ...
    @Override
    public void initializeTextureSize(MBasicTexture texture, int format, int type) {
        int target = texture.getTarget();
        mGL.glBindTexture(target, texture.getId());
    ...
    @Override
    public void initializeTexture(MBasicTexture texture, Bitmap bitmap) {
        int target = texture.getTarget();
        mGL.glBindTexture(target, texture.getId());
    ...
    @Override
    public void texSubImage2D(MBasicTexture texture, int xOffset, int yOffset, Bitmap bitmap,
            int format, int type) {
        int target = texture.getTarget();
        mGL.glBindTexture(target, texture.getId());
    ...
    public void setPerspective(float fovy, float aspect, float zNear, float zFar) {
        ...
        mGL.glLoadIdentity();
        GLU.gluPerspective(mGL, fovy, aspect, zNear, zFar);
        mGL.glMatrixMode(GL11.GL_MODELVIEW);
    }
    ...
    public void setLookAt(float eyeX, float eyeY, float eyeZ, float centerX, float centerY,
        ...
        mGL.glMatrixMode(GL11.GL_MODELVIEW);
        mGL.glLoadIdentity();
        GLU.gluLookAt(mGL, eyeX, eyeY, eyeZ, centerX, centerY, centerZ, upX, upY, upZ);
        float[] matrix = new float[16];
        mGL.glGetFloatv(GL11.GL_MODELVIEW_MATRIX, matrix, 0);
        Matrix.multiplyMM(matrix, 0, matrix, 0, mMatrixValues, 0);
        System.arraycopy(matrix, 0, mMatrixValues, 0, 16);
        mGL.glLoadMatrixf(mMatrixValues, 0);
    }

    @Override
    public void drawMesh(MBasicTexture tex, float degree, float x, float y, float z, int xyzBuffer,
            int uvBuffer, int count) {
        float alpha = mAlpha;
        if (!bindTexture(tex)) {
            return;
        }
        ...
        mGL.glBindBuffer(GL11.GL_ARRAY_BUFFER, xyzBuffer);
        mGL.glVertexPointer(3, GL11.GL_FLOAT, 0, 0);

        mGL.glBindBuffer(GL11.GL_ARRAY_BUFFER, uvBuffer);
        mGL.glTexCoordPointer(2, GL11.GL_FLOAT, 0, 0);

        mGL.glDrawArrays(GL11.GL_TRIANGLES, 0, count);
    ...
    @Override
    public void readPixels(int x, int y, int width, int height, int type, int format, Buffer buffer) {
        mGL.glReadPixels(x, y, width, height, type, format, buffer);
    }
    @Override
    ...
    public void glDeleteBuffers(int n, int[] buffers, int offset) {
        mGL.glDeleteBuffers(n, buffers, offset);
    }
    ...
    class GLES11Generator implements Generator {
        public int generateTexture() {
            int[] tex = new int[1];
            mGL.glGenTextures(1, tex, 0);
            return tex[0];
        }

        public void glGenBuffers(int n, int[] buffers, int offset) {
            mGL.glGenBuffers(n, buffers, offset);
        }
    }

    Generator mGenerator = new GLES11Generator();

    public int generateTexture() {
        return mGenerator.generateTexture();
    }

    public void glGenBuffers(int n, int[] buffers, int offset) {
        mGenerator.glGenBuffers(n, buffers, offset);
    }
