...
public class SelectionManager {
    ...
    */
    ...
    public void toggleTimeShaft(Path path) {
        if (mClickedSet.contains(path)) {
            mClickedSet.remove(path);
        } else {
            enterSelectionMode();
    ...
    public void removeContainsPath(Path path) {
        if (mClickedSet.contains(path)) {
            mClickedSet.remove(path);
        }
    }
    ...
    public void notifySelectionChange() {
        if (mListener != null) mListener.onSelectionChange(null, false);
    }

    public boolean isContainsPath(Path path) {
        return mClickedSet.contains(path);
    }
